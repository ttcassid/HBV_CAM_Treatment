...
ParameterStruc = load('H0731_IndPatientParameters_13June24') ; % load('Study101_IndPatientParameters_FullParticipant_29Jan2024');
PatientParameterMatrix = cell2mat(struct2cell(ParameterStruc));
PatientNumber = 1;
...
elseif HBeAgPos == 0
    ...
    % Population estimates from 0731 fits
    if Trial == 0731
       EpsVec =  [0.907,0.97,0.984]; %  
    else
     % Next-gen estimates 
     PotencyIncreaseFactor = [5,30,500];
     EpsVec = [1-(1-0.984)*(1./PotencyIncreaseFactor(:))]; 
    end
    %% Initial condtions 
...
%     [sol4] =  HBVMultiscaleModel(IC,TotalTime,PA);
    ...
    %     PA.VIC = PA.lambda.*PA.rhoV.*M./PA.cV-PA.dT/PA.beta;
    %     [sol] =  HBVMultiscaleModel(TotalTime,IC,PA);
   
    EquilibVec = zeros(1,length(EpsVec));
    TstarVec =  zeros(1,length(EpsVec));
    TstarVecV1 =  zeros(1,length(EpsVec));
    for ii = 1:length(EpsVec)
        Conv = 0.1; % Convex combination coefficient: 0 is closer to 2nd phase, 1 closer to 1st
...
%     TransparencyValue = 1.0*255;
%     g3 =  plot(sol3.x(:),log10(sol3.y(6,:)./RIC),'LineWidth',1.5,'Color', [67,147,195,TransparencyValue]/256,'LineStyle','-'); %grey
%     hold on
%     g2 =  plot(sol2.x(:),log10(sol2.y(6,:)./RIC),'LineWidth',1.5,'Color', [239,138,98,TransparencyValue]/256,'LineStyle','-'); %grey
%     hold on
...
%     hold on
%     % g4 =  plot(sol4.x(:),log10(sol4.y(6,:)./RIC),'LineWidth',1.5,'Color', [67,147,195,TransparencyValue]/256,'LineStyle','-'); %grey
%     % hold on
%     % g5 =  plot(sol5.x(:),log10(sol5.y(6,:)./RIC),'LineWidth',1.5,'Color', [179,88,6,TransparencyValue]/256,'LineStyle','-'); %grey
%     % hold on
%     g32 =  plot([TstarVecV1(3) TstarVecV1(3)],[-6 0] ,'LineWidth',1.00,'Color',[67,147,195]/256,'LineStyle','--'); %grey
%     hold on
%     g22 =  plot([TstarVecV1(2) TstarVecV1(2)],[-6 0] ,'LineWidth',1.00,'Color',[239,138,98]/256,'LineStyle','--'); %grey
%     hold on
%     g12 =  plot([TstarVecV1(1) TstarVecV1(1)],[-6 0] ,'LineWidth',1.00,'Color',[118,42,131]/255,'LineStyle','--'); %grey
%     hold on
%     % hold on
...
%      PredictedEfficacy1 = 1-deval(sol,TstarVecV1(1),6 )./RIC
%      PredictedEfficacy2 = 1-deval(sol2,TstarVecV1(2),6 )./RIC
%      PredictedEfficacy3 = 1-deval(sol3,TstarVecV1(3),6 )./RIC

%% RNA vs DNA comparison figure
    Fig = figure(1); % PK data  'Color',[152 0 67]./25
    TransparencyValue = 0.8*255;
    g3 =  plot(sol3.x(:),log10(sol3.y(6,:)./RIC),'LineWidth',1.5,'Color', [118,42,131,TransparencyValue]/256,'LineStyle','-'); %grey 67,147,195
    hold on
    g2 =  plot(sol2.x(:),log10(sol2.y(6,:)./RIC),'LineWidth',1.5,'Color', [239,138,98,TransparencyValue]/256,'LineStyle','-'); %grey
    hold on
    g1 =  plot(sol.x(:),log10(sol.y(6,:)./RIC),'LineWidth',1.5,'Color', [67,147,195,TransparencyValue]/255,'LineStyle','-'); %grey % 171,217,233,TransparencyValue
    hold on 
    % g4 =  plot(sol4.x(:),log10(sol4.y(6,:)./RIC),'LineWidth',1.5,'Color', [67,147,195,TransparencyValue]/256,'LineStyle','-'); %grey
    % hold on
    % g5 =  plot(sol5.x(:),log10(sol5.y(6,:)./RIC),'LineWidth',1.5,'Color', [179,88,6,TransparencyValue]/256,'LineStyle','-'); %grey
    % hold on
       % g12 =  plot([TstarVecV1(1) TstarVecV1(1)],[-6 0] ,'LineWidth',1.00,'Color',[67,147,195]/255,'LineStyle','--'); %grey
       % hold on
       % g22 =  plot([TstarVecV1(2) TstarVecV1(2)],[-6 0] ,'LineWidth',1.00,'Color',[239,138,98]/256,'LineStyle','--'); %grey
       % hold on
       % g32 =  plot([TstarVecV1(3) TstarVecV1(3)],[-6 0] ,'LineWidth',1.00,'Color',[118,42,131]/256,'LineStyle','--'); %grey
       % hold on
       % g42 =  plot([TstarVecV1(4) TstarVecV1(4)],[-6 0] ,'LineWidth',0.75,'Color',[67,147,195]/256,'LineStyle','--'); %grey
       % hold on
       % g52 =  plot([TstarVecV1(5) TstarVecV1(5)],[-6 0] ,'LineWidth',0.75,'Color',[179,88,6]/256,'LineStyle','--'); %grey
    %    hold on
%     ylabel('log_{10} change from baseline  ','FontSize',11); % ,'Interpreter','latex','FontSize',15)
%     xlabel('Time (days)','FontSize',12)
    %% HBV DNA Fig
    g3 =  plot(sol3.x(:),log10(sol3.y(5,:)./PA.VIC),'LineWidth',1.5,'Color', [118,42,131] /256,'LineStyle','--'); %grey 67,147,195
    hold on
    g2 =  plot(sol2.x(:),log10(sol2.y(5,:)./PA.VIC),'LineWidth',1.5,'Color', [239,138,98]/256,'LineStyle','--'); %grey
    hold on
    g1 =  plot(sol.x(:),log10(sol.y(5,:)./PA.VIC),'LineWidth',1.5,'Color', [67,147,195]/255,'LineStyle','--'); %grey [171,217,233]
    hold on
    % g4 =  plot(sol4.x(:),log10(sol4.y(5,:)./PA.VIC),'LineWidth',1.5,'Color', [67,147,195]/256,'LineStyle','--'); %grey
    % hold on
    % g5 =  plot(sol5.x(:),log10(sol5.y(5,:)./PA.VIC),'LineWidth',1.5,'Color', [179,88,6]/256,'LineStyle','--'); %grey
    % hold on  
    if Trial == 0731
     ylim([-2.5 0])
    else
...
%% pgRNA vs rcDNA
    Fig = figure(PatientID(PatientNumber)+PatientID(end) ); % PK data  'Color',[152 0 67]./25
    TransparencyValue = 0.8*255;
    g3 =  plot(sol3.x(:),log10(sol3.y(3,:)./PIC),'LineWidth',1.5,'Color', [67,147,195,TransparencyValue]/256,'LineStyle','-'); %grey
    hold on
    g2 =  plot(sol2.x(:),log10(sol2.y(3,:)./PIC),'LineWidth',1.5,'Color', [239,138,98,TransparencyValue]/256,'LineStyle','-'); %grey
    hold on
    g1 =  plot(sol.x(:),log10(sol.y(3,:)./PIC),'LineWidth',1.5,'Color', [118,42,131,TransparencyValue]/255,'LineStyle','-'); %grey % 171,217,233,TransparencyValue
    hold on 
    %%  rcDNA Fig
    g3 =  plot(sol3.x(:),log10(sol3.y(4,:)./QIC),'LineWidth',1.5,'Color', [67,147,195] /256,'LineStyle','--'); %grey
    hold on
    g2 =  plot(sol2.x(:),log10(sol2.y(4,:)./QIC),'LineWidth',1.5,'Color', [239,138,98]/256,'LineStyle','--'); %grey
    hold on
    g1 =  plot(sol.x(:),log10(sol.y(4,:)./QIC),'LineWidth',1.5,'Color', [118,42,131]/255,'LineStyle','--'); %grey [171,217,233]
    hold on 
     ylim([-2.5 0]) 
