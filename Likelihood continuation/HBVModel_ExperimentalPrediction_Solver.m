function [ModelValues] =  HBVModel_ExperimentalPrediction_Solver(X,D,TimeVec,Y);
...
HBVDNAModelObjFunc = zeros(1,NDataPoints); 
HBVDNAModelObjFunc(~isnan(DNAViralLoadData)>0) =  log10( EvalSolHBVDNA(~isnan(DNAViralLoadData))./5.82 );  %  log10( EvalSolHBVDNA(~isnan(DNAViralLoadData))./5.82 ) -log10( EvalSolHBVDNA(1)) ; %

HBVRNAModelObjFunc = zeros(1,NDataPoints); 
HBVRNAModelObjFunc(~isnan(RNAViralLoadData)>0) = log10(EvalSolHBVRNA(~isnan(RNAViralLoadData) ) ); % log10(EvalSolHBVRNA(~isnan(RNAViralLoadData) ) )-log10( EvalSolHBVRNA(1)); % 

ModelValues = [HBVDNAModelObjFunc,HBVRNAModelObjFunc]; % [log10( EvalSolHBVDNA./5.82 ) , log10(EvalSolHBVRNA ) ]; %Convert copies to IU/mL.
